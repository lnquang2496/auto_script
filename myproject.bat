...
REM call python myproject.py --idir %PCL_PATH% --ifile %PCL_NAME% --wsheet r_imr_drvknl_iodev_write_status --source %SRC_NAME% --check_seq True --can_dir %CANTATA_DIR%
...
call set PCL_NAME=IMR_INT_V3H_B_V1_unit_test_specification_imr_OSAL.xlsx
call set TP_DIR=UTP_IMR_OSAL
call set CANTATA_DIR=C:\Users\quangla\workspace\%TP_DIR%\Cantata\tests
call set SRC_NAME=r_imr_osal_ctl
call python reset_stub_instance.py --can_dir %CANTATA_DIR% --source %SRC_NAME%
call python myproject.py --idir %PCL_PATH% --ifile %PCL_NAME% --wsheet r_imr_osal_ctl_init --source %SRC_NAME% --check_seq False --can_dir %CANTATA_DIR%
call python myproject.py --idir %PCL_PATH% --ifile %PCL_NAME% --wsheet r_imr_osal_ctl_deinit --source %SRC_NAME% --check_seq False --can_dir %CANTATA_DIR%
call python myproject.py --idir %PCL_PATH% --ifile %PCL_NAME% --wsheet r_imr_osal_ctl_start --source %SRC_NAME% --check_seq True --can_dir %CANTATA_DIR%
call python myproject.py --idir %PCL_PATH% --ifile %PCL_NAME% --wsheet r_imr_osal_ctl_stop --source %SRC_NAME% --check_seq True --can_dir %CANTATA_DIR%
call python myproject.py --idir %PCL_PATH% --ifile %PCL_NAME% --wsheet r_imr_osal_ctl_get_iodevice --source %SRC_NAME% --check_seq True --can_dir %CANTATA_DIR%
